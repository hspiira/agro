...
function StatCard({ stat, index }: { stat: (typeof heroStats)[number]; index: number }) {
  ...
  return (
    <div ref={ref} className="text-center px-3 py-2 md:px-6 md:py-4">
      <div className="text-2xl md:text-4xl font-bold font-money text-[var(--m-accent)]">
        {value}
        {stat.value.includes('%') && '%'}
        {stat.value.includes('+') && '+'}
      </div>
      <p className="text-xs md:text-sm text-[var(--m-sub)] mt-1">{stat.label}</p>
    </div>
  );
}
...
function StatsSection() {
  return (
    <div className="px-4 md:px-12 mb-8 md:mb-12">
      <div className="bg-[var(--m-s1)] rounded-xl p-4 md:p-6 flex justify-between md:justify-around overflow-x-auto gap-2 border border-[var(--m-border)]">
